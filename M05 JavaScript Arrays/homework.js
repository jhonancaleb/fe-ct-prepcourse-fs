...
function agregarNumeros(arrayOfNums) {
   ...
   // Tu código:
   return arrayOfNums.reduce((ac = 0, num) => ac + num);
}
...
function promedioResultadosTest(resultadosTest = []) {
   ...
   // Tu código:
   const suma = resultadosTest.reduce((ac, num) => ac + num, 0)
...
function mesesDelAño(array) {
   ...
   const mesesEnc = []
   array.forEach(mes => {
      if (meses.includes(mes)) mesesEnc.push(mes)
   })

   if (mesesEnc.length === 3) return mesesEnc
...
function breakStatement(num) {
   ...
   for (let i = 1; i <= 10; i++) {
      num = num + 2
      if (num === i) {
         ret = "Se interrumpió la ejecución"
         break
      }
